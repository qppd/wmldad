...
|-------|----------|----------|
| **normal** (0) | Typical usage: faucet, flush, shower | Flow > 1 L/min, duration 10s-30min, matches fixture pattern |
    ...
    for col in ['inlet', 'fixture_1', 'fixture_2', 'fixture_3']:
        for metric in ['flow_rate', 'volume', 'total']:
            ...
            if key in df.columns:
                df[key] = df[key].clip(lower=0, upper=50)
...
Already defined in [Dataset Planning](#feature-set-9-features). Implementation:
