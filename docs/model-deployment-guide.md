...
> **Target:** Raspberry Pi 3B+/4/5 running Raspberry Pi OS Trixie (64-bit)  
...
2. [Model Export from Training](#model-export-from-training)
3. [RPi Environment Setup](#rpi-environment-setup)
4. [Model Loading & Inference](#model-loading--inference)
5. [Performance Optimization](#performance-optimization)
6. [Flask API Integration](#flask-api-integration)
7. [Monitoring & Logging](#monitoring--logging)
8. [Model Updates & Versioning](#model-updates--versioning)
...
class LeakDetector:
    ...
    def _load_models(self):
        ...
        try:
            ...
            self.xgb.load_model(self.model_dir / 'xgboost_model.json')
            ...
            # Threshold
            self.iso_threshold = joblib.load(self.model_dir / 'iso_threshold.pkl')
    ...
    def predict(self, features_raw):
        ...
        Args:
            features_raw: np.array of shape (9,) or (1, 9) or (n, 9)
        ...
        # XGBoost inference
        xgb_proba = self.xgb.predict_proba(features_scaled)
        xgb_pred = np.argmax(xgb_proba, axis=1)
        xgb_conf = np.max(xgb_proba, axis=1)
        
        # Isolation Forest
        iso_scores = self.iso_forest.score_samples(features_scaled)
        iso_anomaly = iso_scores < self.iso_threshold
        ...
        for i in range(len(features)):
            result = {
                'xgboost': {
                    'class': self.target_names[xgb_pred[i]],
                    'confidence': float(xgb_conf[i]),
                    'probabilities': {
                        name: float(xgb_proba[i, j]) 
                        for j, name in enumerate(self.target_names)
                    }
                },
                'isolation_forest': {
                    'anomaly': bool(iso_anomaly[i]),
                    'score': float(iso_scores[i])
                }
            }
            
            # Decision logic
            if xgb_conf[i] >= self.confidence_threshold:
                result['final'] = result['xgboost']['class']
                result['confidence'] = float(xgb_conf[i])
            elif iso_anomaly[i]:
                result['final'] = 'anomaly'
                result['confidence'] = float(1.0 - abs(iso_scores[i]))
            else:
    ...
    def warm_up(self):
        """Run dummy inference to warm up (JIT, cache)"""
        dummy = np.zeros((1, self.n_features), dtype=np.float32)
        _ = self.predict(dummy)
        logger.info("🔥 Warm-up complete")

# Global instance (singleton pattern for Flask)
_detector = None

def get_detector(model_dir='models'):
    global _detector
    if _detector is None:
        _detector = LeakDetector(model_dir)
        _detector.warm_up()
    return _detector
```
...
def extract_features_from_request(data):
    ...
    # Determine which fixture this reading is for
    # (implementation depends on your data structure)
    # ...
    return np.array([...])  # 9 features
```
...
*Last updated: July 2026 | XGBoost 2.0.3 | scikit-learn 1.3.2 | Raspberry Pi OS Trixie (64-bit) | Tested on Pi 4 (8GB) and Pi 5 (8GB)*